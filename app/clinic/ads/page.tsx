...
const EMPTY: AdForm = {
  ...
  badge: 'SPONSORED', gradientFrom: '#10b981', gradientTo: '#059669',
...
function downloadAdBill(ad: Ad) {
  ...
  const lines = [
    ...
    `Status:       ${STATUS_CFG[ad.status]?.label || ad.status}`,
    `Plan:         ${budget.label}`,
  ...
  a.download = `carebridge-clinic-ad-bill-${ad._id.slice(-8)}.txt`
  a.click()
}

export default function ClinicAdsPage() {
  ...
  const handleSubmit = async () => {
    setSubmitting(true)
    try {
      await axios.post('/api/clinic/ads', form)
      const r = await axios.get('/api/clinic/ads')
      setAds(r.data?.ads || [])
      setStep('done')
  ...
  // Reusable live preview card
  const AdPreview = ({ a, small }: { a: AdForm | Ad; small?: boolean }) => {
    const gF = (a as AdForm).gradientFrom || (a as Ad).gradientFrom || '#10b981'
    const gT = (a as AdForm).gradientTo   || (a as Ad).gradientTo   || '#059669'
    const ct = (a as AdForm).ctaText      || (a as Ad).ctaText      || 'Learn More'
    const w  = small ? 220 : 280
    return (
      <div style={{ borderRadius: 20, overflow: 'hidden', width: w, background: `linear-gradient(135deg,${gF},${gT})` }}>
        <div style={{ padding: 16, position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ fontSize: small ? 13 : 15, fontWeight: 800, color: '#fff', marginBottom: 4 }}>{a.title || 'Your Clinic Name'}</div>
  ...
  // Inline styles matching hospital panel design system
          ...
          <div style={{ flex: 1, padding: 24, overflowY: 'auto', background: '#F8FAFC' }}>
              ...
              <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #E2E8F0', marginBottom: 20, overflow: 'hidden' }}>
                <div style={{ padding: '14px 20px', borderBottom: '1px solid #E2E8F0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  <button onClick={() => { setStep('poster'); setForm(EMPTY); setShowForm(true) }}
                    style={{ padding: '8px 16px', background: '#10b981', color: '#fff', border: 'none', borderRadius: 10, fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>
                    + New Ad
                  </button>
                </div>
                          ...
                          <tr key={ad._id} style={{ borderTop: '1px solid #F1F5F9' }}>
                            <td style={{ padding: '12px 16px' }}>
                              <button onClick={() => setDetailAd(ad)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0, textAlign: 'left' }}>
                                <div style={{ fontSize: 13, fontWeight: 700, color: '#10b981', textDecoration: 'underline', marginBottom: 2 }}>{ad.title}</div>
                            ...
                            <td style={{ padding: '12px 16px', color: '#10b981', fontWeight: 700, whiteSpace: 'nowrap' }}>Rs. {ad.budget?.toLocaleString()}</td>
                            ...
                            <td style={{ padding: '12px 16px' }}>
                              <div style={{ display: 'flex', gap: 6 }}>
                                <button onClick={() => setDetailAd(ad)} style={{ padding: '5px 10px', background: '#D1FAE5', color: '#065f46', border: 'none', borderRadius: 8, fontSize: 11, fontWeight: 600, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>Details</button>
                                ...
                                {['pending','approved'].includes(ad.status) && (
                                  <button onClick={() => setCancelAd(ad)} style={{ padding: '5px 10px', background: '#FEE2E2', color: '#DC2626', border: 'none', borderRadius: 8, fontSize: 11, fontWeight: 600, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>Cancel</button>
                                )}
                              </div>
                            </td>
                          </tr>
                    ...
                    {STEPS_META.map((s, i) => (
                      <div key={s.key} style={{ display: 'flex', alignItems: 'center', flex: i < STEPS_META.length - 1 ? 1 : 0 }}>
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                          <div style={{ width: 28, height: 28, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 700, background: i <= stepIdx ? '#10b981' : '#F1F5F9', color: i <= stepIdx ? '#fff' : '#94A3B8' }}>
                            {i < stepIdx ? '✓' : i + 1}
                          </div>
                          <div style={{ fontSize: 10, fontWeight: 600, color: i <= stepIdx ? '#10b981' : '#94A3B8', whiteSpace: 'nowrap' }}>{s.label}</div>
                        </div>
                        {i < STEPS_META.length - 1 && <div style={{ flex: 1, height: 2, background: i < stepIdx ? '#10b981' : '#E2E8F0', margin: '0 8px', marginBottom: 16 }} />}
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div style={{ padding: 28 }}>
                  ...
                  {step === 'poster' && (
                    <div>
                      ...
                      <div style={{ fontSize: 13, color: '#64748B', marginBottom: 24 }}>Choose colors and badge for your clinic ad banner</div>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
                        <div>
                          <div style={{ marginBottom: 16 }}>
                            <label style={lbl}>Badge Text</label>
                            <select value={form.badge} onChange={e => up('badge', e.target.value)} style={{ ...inp }}>
                              <option>SPONSORED</option><option>SPECIAL OFFER</option><option>FREE CHECKUP</option><option>NEW</option><option>EMERGENCY</option>
                            </select>
                          </div>
                          <div style={{ marginBottom: 16 }}>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 24 }}>
                        <button onClick={() => setStep('details')} style={{ padding: '12px 28px', background: '#10b981', color: '#fff', border: 'none', borderRadius: 12, fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>Next: Ad Details →</button>
                      </div>
                    </div>
                  )}
                    ...
                    <div>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
                        <div style={{ display: 'grid', gap: 16 }}>
                          <div><label style={lbl}>Clinic / Ad Title *</label><input style={inp} placeholder="e.g. HealthFirst Clinic – Free Consultation" value={form.title} onChange={e => up('title', e.target.value)} /></div>
                          <div><label style={lbl}>Subtitle</label><input style={inp} placeholder="e.g. Book your slot today" value={form.subtitle} onChange={e => up('subtitle', e.target.value)} /></div>
                          <div>
                          ...
                          <div><label style={lbl}>Website (optional)</label><input style={inp} placeholder="https://yourclinic.com" value={form.targetUrl} onChange={e => up('targetUrl', e.target.value)} /></div>
                        </div>
                        <div>
                          <label style={lbl}>Live Preview</label>
                          <AdPreview a={form} />
                      ...
                      <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end', marginTop: 24 }}>
                        ...
                        <button onClick={() => setStep('content')} disabled={!form.title} style={{ padding: '12px 28px', background: form.title ? '#10b981' : '#E2E8F0', color: form.title ? '#fff' : '#94A3B8', border: 'none', borderRadius: 12, fontSize: 14, fontWeight: 700, cursor: form.title ? 'pointer' : 'not-allowed', fontFamily: 'DM Sans, sans-serif' }}>Next: Content →</button>
                      </div>
                    </div>
                  )}
                  ...
                  {step === 'content' && (
                    <div>
                      ...
                      <div style={{ fontSize: 13, color: '#64748B', marginBottom: 24 }}>Add a message and choose your budget plan</div>
                      <div style={{ marginBottom: 20 }}>
                        <label style={lbl}>Message to CareBridge Team</label>
                        <textarea placeholder="Describe your clinic, services you want to promote, and any special instructions..." value={form.message} onChange={e => up('message', e.target.value)} style={{ ...inp, height: 100, resize: 'none' } as React.CSSProperties} />
                      </div>
                      <div style={{ marginBottom: 24 }}>
                        <label style={lbl}>Select Budget Plan</label>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12 }}>
                          {BUDGETS.map(b => (
                            <div key={b.amount} onClick={() => up('budget', b.amount)} style={{ border: `2px solid ${form.budget === b.amount ? '#10b981' : '#E2E8F0'}`, borderRadius: 12, padding: 14, cursor: 'pointer', background: form.budget === b.amount ? '#D1FAE5' : '#fff', transition: 'all 0.15s' }}>
                              <div style={{ fontSize: 12, fontWeight: 700, color: form.budget === b.amount ? '#10b981' : '#64748B', marginBottom: 4 }}>{b.label}</div>
                      ...
                      <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end' }}>
                        ...
                        <button onClick={() => setStep('preview')} style={{ padding: '12px 28px', background: '#10b981', color: '#fff', border: 'none', borderRadius: 12, fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>Preview Ad →</button>
                      </div>
                    </div>
                  )}
                    ...
                    <div>
                      ...
                      <div style={{ background: '#F8FAFC', borderRadius: 12, padding: 16, marginBottom: 24, border: '1px solid #E2E8F0' }}>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 12, paddingTop: 12, borderTop: '2px solid #E2E8F0' }}>
                          ...
                          <span style={{ fontSize: 18, fontWeight: 900, color: '#10b981' }}>Rs. {selectedBudget.amount.toLocaleString()}</span>
                        </div>
                      </div>
                      <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end' }}>
                        ...
                        <button onClick={() => setStep('payment')} style={{ padding: '12px 28px', background: '#10b981', color: '#fff', border: 'none', borderRadius: 12, fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>Proceed to Payment →</button>
                      </div>
                    </div>
                  )}
                  ...
                  {step === 'payment' && (
                    <div>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
                        <div>
                          <div style={{ background: 'linear-gradient(135deg,#064e3b,#065f46)', borderRadius: 16, padding: 20, marginBottom: 20, color: '#fff' }}>
                            <div style={{ fontSize: 11, opacity: 0.5, marginBottom: 20 }}>CAREBRIDGE CLINIC ADVERTISING</div>
                        ...
                        <div>
                          <div style={{ background: '#F8FAFC', borderRadius: 16, padding: 20, border: '1px solid #E2E8F0' }}>
                            ...
                            {[{ label: 'Ad Campaign', value: form.title }, { label: 'Plan', value: selectedBudget.label }, { label: 'Duration', value: `${selectedBudget.days} days` }, { label: 'Reach', value: selectedBudget.reach }].map((r,i) => (
                              <div key={i} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
                                <span style={{ fontSize: 13, color: '#64748B' }}>{r.label}</span>
                                <span style={{ fontSize: 13, fontWeight: 600, color: i===3?'#10b981':'#0F172A' }}>{r.value}</span>
                              </div>
                            ))}
                            <div style={{ borderTop: '2px solid #E2E8F0', marginTop: 12, paddingTop: 12, display: 'flex', justifyContent: 'space-between' }}>
                              ...
                              <span style={{ fontSize: 20, fontWeight: 900, color: '#10b981' }}>Rs. {selectedBudget.amount.toLocaleString()}</span>
                            </div>
                          </div>
                          <div style={{ background: '#D1FAE5', borderRadius: 12, padding: 14, marginTop: 12, border: '1px solid #6ee7b7' }}>
                            <div style={{ fontSize: 12, color: '#065f46', lineHeight: 1.5 }}>Your clinic ad will be reviewed within 24 hours. Unused balance refunded on cancellation.</div>
                          </div>
                        </div>
                      </div>
                      <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end', marginTop: 24 }}>
                        ...
                        <button onClick={handleSubmit} disabled={submitting || cardNo.length < 16 || !cardName || expiry.length < 5 || cvv.length < 3}
                          style={{ padding: '12px 32px', background: (cardNo.length>=16&&cardName&&expiry.length>=5&&cvv.length>=3&&!submitting)?'#10b981':'#E2E8F0', color: (cardNo.length>=16&&cardName&&expiry.length>=5&&cvv.length>=3&&!submitting)?'#fff':'#94A3B8', border: 'none', borderRadius: 12, fontSize: 14, fontWeight: 700, cursor: (cardNo.length>=16&&cardName&&expiry.length>=5&&cvv.length>=3&&!submitting)?'pointer':'not-allowed', fontFamily: 'DM Sans, sans-serif' }}>
            ...
            {step === 'done' && (
              <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #E2E8F0', padding: 48, textAlign: 'center' }}>
                ...
                <div style={{ fontSize: 14, color: '#64748B', marginBottom: 24, maxWidth: 400, margin: '0 auto 24px' }}>Your clinic ad request has been received. The CareBridge team will review it within 24 hours.</div>
                <button onClick={() => { setStep('poster'); setForm(EMPTY); setShowForm(false) }} style={{ padding: '12px 28px', background: '#10b981', color: '#fff', border: 'none', borderRadius: 12, fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>View My Ads</button>
              </div>
            )}
          </div>
        </main>
            ...
            <div onClick={e => e.stopPropagation()} style={{ background: '#fff', borderRadius: 24, width: '100%', maxWidth: 560, maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 24px 80px rgba(0,0,0,0.3)' }}>
              ...
              <div style={{ padding: 24, display: 'grid', gap: 20 }}>
                ...
                <div style={{ background: '#F8FAFC', borderRadius: 12, padding: 16, border: '1px solid #E2E8F0' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}><span style={{ fontSize: 12, color: '#64748B' }}>Day {daysRun} of {totalDays}</span><span style={{ fontSize: 12, fontWeight: 700, color: '#10b981' }}>{Math.min(100,Math.round((daysRun/totalDays)*100))}% complete</span></div>
                  <div style={{ background: '#E2E8F0', borderRadius: 6, height: 8, overflow: 'hidden' }}><div style={{ width: `${Math.min(100,Math.round((daysRun/totalDays)*100))}%`, height: '100%', background: 'linear-gradient(90deg,#10b981,#34d399)', borderRadius: 6 }} /></div>
                ...
                {cancelDone ? (
                  ...
                  <>
                    <div style={{ background: '#F8FAFC', borderRadius: 12, padding: 16, marginBottom: 20, border: '1px solid #E2E8F0' }}>
                      <div style={{ fontSize: 13, fontWeight: 700, color: '#0F172A', marginBottom: 12 }}>Billing Breakdown</div>
                    ...
                    <div style={{ display: 'flex', gap: 12 }}>
                      ...
                      <button onClick={handleCancel} disabled={cancelling} style={{ flex: 1, padding: 12, background: cancelling?'#94A3B8':'#DC2626', color: '#fff', border: 'none', borderRadius: 12, fontSize: 13, fontWeight: 700, cursor: cancelling?'not-allowed':'pointer', fontFamily: 'DM Sans, sans-serif' }}>
