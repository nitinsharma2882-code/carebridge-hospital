...
function downloadCSV(bookings: Booking[]) {
  ...
  a.download = `carebridge-clinic-bookings-${Date.now()}.csv`; a.click()
  URL.revokeObjectURL(url)
}

function downloadPDF(bookings: Booking[]) {
  const lines: string[] = [
    'CAREBRIDGE CLINIC BOOKINGS REPORT',
  ...
  const url  = URL.createObjectURL(blob)
  const a    = document.createElement('a'); a.href = url
  a.download = `carebridge-clinic-bookings-${Date.now()}.txt`; a.click()
  URL.revokeObjectURL(url)
}

function isUpcoming(b: Booking): boolean {
  if (!b.date) return false
  return new Date(b.date) > new Date() && !['completed', 'cancelled'].includes(b.status)
}

export default function ClinicBookingsPage() {
  const [bookings, setBookings] = useState<Booking[]>([])
  const [loading,  setLoading]  = useState(true)
  const [tab,      setTab]      = useState<'all' | 'upcoming' | 'pending' | 'completed' | 'cancelled'>('all')
  const [selected, setSelected] = useState<Booking | null>(null)
  const [search,   setSearch]   = useState('')

  useEffect(() => {
    axios.get('/api/clinic/bookings')
      .then(res => {
        const data = res.data
        ...
        <div style={{ marginLeft: 256, flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
            ...
            actions={
              <>
                ...
                <button onClick={() => downloadPDF(filtered)}
                  style={{ padding: '8px 14px', background: '#10b981', color: '#fff', border: 'none', borderRadius: 10, fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>
                  Download PDF
                </button>
              </>
            }
          ...
          <div style={{ flex: 1, padding: 24, overflowY: 'auto' }}>
            ...
            {upcomingBookings.length > 0 && tab !== 'upcoming' && (
              <div onClick={() => setTab('upcoming')}
                style={{ background: 'linear-gradient(135deg,#059669,#10b981)', borderRadius: 14, padding: '14px 20px', marginBottom: 16, display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' }}>
            ...
            <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #E2E8F0', marginBottom: 16, overflow: 'hidden' }}>
              ...
              <div style={{ display: 'flex', borderBottom: '1px solid #E2E8F0', overflowX: 'auto' }}>
                {tabs.map(t => (
                  <button key={t.key} onClick={() => setTab(t.key)}
                    style={{ flex: 1, minWidth: 80, padding: '12px 8px', border: 'none', background: 'none', cursor: 'pointer', fontSize: 12, fontWeight: tab === t.key ? 700 : 500, color: tab === t.key ? '#10b981' : '#94A3B8', borderBottom: tab === t.key ? '2.5px solid #10b981' : '2.5px solid transparent', fontFamily: 'DM Sans, sans-serif', whiteSpace: 'nowrap' }}>
                    {t.label} <span style={{ marginLeft: 4, background: tab === t.key ? '#D1FAE5' : '#F1F5F9', color: tab === t.key ? '#10b981' : '#94A3B8', fontSize: 11, fontWeight: 700, borderRadius: 10, padding: '1px 6px' }}>{t.count}</span>
                  </button>
                ))}
              </div>
              ...
              {tab === 'upcoming' && (
                <div style={{ padding: 20 }}>
                  {filtered.length === 0 ? (
                    <div style={{ textAlign: 'center', padding: 40, color: '#94A3B8', fontSize: 14 }}>No upcoming bookings scheduled</div>
                  ) : (
                    <div style={{ display: 'grid', gap: 12 }}>
                      {filtered
                        ...
                        .map(b => {
                          ...
                          return (
                            <div key={b._id} style={{ border: '1.5px solid #D1FAE5', borderRadius: 12, padding: 16, background: '#F0FDF4', display: 'flex', alignItems: 'center', gap: 16 }}>
                              <div style={{ flexShrink: 0, width: 56, height: 56, borderRadius: 12, background: '#059669', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#fff' }}>
                              ...
                              <div style={{ flex: 1, minWidth: 0 }}>
                                ...
                                <div style={{ fontSize: 12, color: '#475569', marginTop: 2 }}>{b.userId?.name || b.patientName || 'Patient'} • {b.hospital || b.destinationHospital || 'N/A'}</div>
                              ...
                              <div style={{ flexShrink: 0, textAlign: 'right' }}>
                                <div style={{ fontSize: 14, fontWeight: 700, color: '#10b981' }}>Rs. {b.fare}</div>
                                ...
                                <div style={{ marginTop: 8 }}>
                                  <button onClick={() => setSelected(b)} style={{ padding: '5px 12px', background: '#D1FAE5', color: '#059669', border: 'none', borderRadius: 8, fontSize: 11, fontWeight: 600, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>View</button>
                                </div>
                              </div>
                            </div>
                          )
                        ...
                        {filtered.map(b => {
                          ...
                          return (
                            <tr key={b._id} style={{ borderTop: '1px solid #F1F5F9' }}>
                              ...
                              <td style={{ padding: '12px 16px', color: '#10b981', fontWeight: 700, whiteSpace: 'nowrap' }}>Rs. {b.fare}</td>
                              ...
                              <td style={{ padding: '12px 16px' }}>
                                <div style={{ display: 'flex', gap: 6 }}>
                                  <button onClick={() => setSelected(b)} style={{ padding: '5px 10px', background: '#D1FAE5', color: '#059669', border: 'none', borderRadius: 8, fontSize: 11, fontWeight: 600, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>View</button>
                                  <button onClick={() => downloadPDF([b])} style={{ padding: '5px 10px', background: '#F1F5F9', color: '#475569', border: 'none', borderRadius: 8, fontSize: 11, fontWeight: 600, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>Download</button>
                                  <button onClick={() => shareBooking(b)} style={{ padding: '5px 10px', background: '#F1F5F9', color: '#475569', border: 'none', borderRadius: 8, fontSize: 11, fontWeight: 600, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>Share</button>
                                </div>
                              </td>
                            </tr>
                          )
            ...
            <div onClick={e => e.stopPropagation()}
              ...
              <div style={{ padding: '20px 24px', borderBottom: '1px solid #E2E8F0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                  ...
                  <button onClick={() => setSelected(null)} style={{ width: 32, height: 32, borderRadius: 8, background: '#F1F5F9', border: 'none', cursor: 'pointer', fontSize: 16, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
                </div>
              </div>
              <div style={{ padding: 24 }}>
                ...
                <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
                  <button onClick={() => downloadPDF([selected])} style={{ flex: 1, padding: 12, background: '#F1F5F9', color: '#475569', border: 'none', borderRadius: 12, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>Download</button>
                  <button onClick={() => downloadCSV([selected])} style={{ flex: 1, padding: 12, background: '#F1F5F9', color: '#475569', border: 'none', borderRadius: 12, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>Export CSV</button>
                  <button onClick={() => shareBooking(selected)} style={{ flex: 1, padding: 12, background: '#10b981', color: '#fff', border: 'none', borderRadius: 12, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>Share</button>
                </div>
              </div>
            </div>
          </div>
